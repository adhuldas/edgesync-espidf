/**
 * EdgeSync over the built-in MQTT transport.
 *
 * Connects to Wi-Fi (see `idf.py menuconfig` -> "EdgeSync MQTT Example" for
 * SSID/password/broker/topic), then publishes a small JSON payload every
 * few seconds. Because edgesync_publish() never blocks on the network,
 * messages queue up durably even before the network finishes connecting, or
 * if it drops later - EdgeSync drains them as connectivity allows.
 *
 * This transport only talks to whatever IP network is up - swap
 * wifi_init_station() below for an esp_modem PPP bring-up over a cellular
 * modem and the rest of this file (transport config, publish loop) is
 * unchanged. See TRANSPORTS.md for details.
 */
#include <string.h>
#include <stdio.h>
...
void app_main(void)
{
    ...
    edgesync_mqtt_transport_config_t mqtt_cfg = {
        ...
        .topic = CONFIG_EXAMPLE_MQTT_TOPIC,
        .qos = 1,
    };
